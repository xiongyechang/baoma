        ...
        <template class="custom-tree-node" #default="{ data }">
          ...
          <template v-else>
            <div class="flex-center-start">
              ...
              <img :src="data.avatar" @click.stop="setCategoryAvatar(data)" height="16" width="16">
  ...
  export default {
    ...
    setup(props, context) {
      ...
      const getCodeSnippets = async () => {
        try {
          const { code, message, data: { rows, count } } = await API.getCodeSnippets(_data.page, _data.limit);
      ...
      const getCodeSnippetsByCategory = async (_id) => {
        ...
        try {
          if (_id === root.data._id) {
            ...
            response = await API.getCodeSnippetsByCategory({ _id, page: _data.page, limit: _data.limit })
          }

          const { code, message, data: { count, rows } } = response
